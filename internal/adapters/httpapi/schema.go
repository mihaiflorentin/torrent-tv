...
func SettingsSchema(s *config.Store) []SchemaField {
	fields := []SchemaField{
		...
		{Key: "metadataLanguage", Label: "Metadata language", Help: "Language TMDB uses for titles, overviews, and artwork text, for example ro-RO. Region-coded values select regional translations and artwork."},
		{Key: "metadataFallbackLanguage", Label: "Metadata fallback language", Help: "Language used when a title has no metadata in the metadata language, for example en-US."},
		{Key: "metadataProviders", Label: "Metadata providers (priority order)", Help: "Comma-separated provider ids tried in order: tmdb (needs API key), tvmaze (TV, keyless), jikan (anime, keyless).", TVVisible: false},
		...
		{Key: "torrentPublicPeerPort", Label: "Torrent public peer port", Help: "Port the built-in engine's public-discovery listener uses. 0 lets the OS pick one. Changing it requires restart.", RestartRequired: true},
		...
		{Key: "subtitleCacheMaxBytes", Label: "Subtitle cache maximum", Help: "Maximum disk space used by prepared and downloaded subtitle files, entered in gigabytes (GiB)."},
		...
		{Key: "initialBufferBytes", Label: "Initial buffer", Help: "Amount probed for media details before an in-progress download answers the info request, entered in megabytes (MiB). Larger values improve reliability but delay startup.", TVVisible: true},
		...
		{Key: "readAheadBytes", Label: "Read-ahead", Help: "Range prioritized ahead of the current playback position, entered in megabytes (MiB).", TVVisible: true},
