...
import { householdSections, nearEnd, trackerCategories } from './catalog-data';
...
export function Setup({ draft, server, status, onDraft, onConnect, onForget }: { draft: string; server: string; status: string; onDraft: (value: string) => void; onConnect: (url: string) => void; onForget: () => void }) {
 ...
 const scan = async () => {
  ...
  try {
   ...
   if (!info?.ip || !info.subnetMask) throw new Error('Automatic discovery is unavailable on this device. Use Manual address.');
   let customPort = 0;
   try { customPort = Number(new URL(normalizeServerURL(draft)).port || (/^https:/i.test(draft) ? 443 : 80)) } catch { }
   const found = await discoverServers(info.ip, info.subnetMask, [8097, customPort], (done, total) => {
    if (generation !== scanGeneration.current) return;
    if (done === total || done % 32 === 0) setDiscoveryStatus(`Searching your network… ${done} of ${total}`);
   });
   if (generation !== scanGeneration.current) return;
   setResults(found);
   setDiscoveryStatus(found.length ? `${found.length} Torrent TV server${found.length === 1 ? '' : 's'} found.` : 'No Torrent TV server was found. Check that it is running or use Manual address.');
  } catch (error) {
   if (generation !== scanGeneration.current) return;
   setDiscoveryStatus((error as Error).message);
   setManual(true);
  } finally {
   if (generation === scanGeneration.current) setSearching(false);
  }
 };
...
type TVRoute = 'home' | 'search' | 'library' | 'continue' | 'favorites' | 'watched' | 'downloads' | 'library-categories' | 'tracker' | 'browse' | 'categories' | 'jobs' | 'events' | 'settings' | 'projects';
...
export function Catalog({ api, status, titles, facets, household, downloads, jobs, restoreFocus, portal, updateStatus, onUpdateStatus, onFocus, onRetry, onChangeServer, onForgetServer, onPlay, onPlayDownload, onManageDownload, onManageSeasonPack, onRefreshDownloads, onFavorite, preparing, onCancelPrepare, onRetryPrepare }: { api: API; status: string; titles: CatalogTitle[]; facets: CatalogFacets; household: HouseholdState; downloads: Download[]; jobs: Job[]; restoreFocus: string | null; portal: PortalState | null; updateStatus: UpdateStatus | null; onUpdateStatus: (status: UpdateStatus) => void; onFocus: (key: string) => void; onRetry: () => void; onChangeServer: () => void; onForgetServer: () => void; onPlay: (release: Release, fileIndex?: number, resumeMs?: number) => void; onPlayDownload: (download: Download) => void; onManageDownload: (download: Download, action: string) => Promise<void>; onManageSeasonPack: (source: CatalogSource, season: number, action: SeasonPackAction) => Promise<void | boolean>; onRefreshDownloads: () => Promise<void>; onFavorite: (title: CatalogTitle, value: boolean) => void; preparing?: TVPreparingState | null; onCancelPrepare?: () => void; onRetryPrepare?: () => void }) {
 ...
 // App-level titles only change identity on a full snapshot refresh
 // (connect, visible-return): replace the held pages so fresh snapshot
 // titles surface immediately and accumulated pages release with it.
 useEffect(() => setRemoteTitles(titles.slice()), [titles]);
 const fetchPage = (cursor = '') => api.titles({ search: query.trim().length >= 3 ? query.trim() : undefined, category, sort, pageSize: 12, cursor }).then(page => { setRemoteTitles(page.items); setNextCursor(page.nextCursor); void api.ensureMetadata(page.items.map(item => item.id)); return page; });
 useEffect(() => { void fetchPage('').catch(() => { }) }, [query, category, sort]);
 // Entering a catalog view with nothing held loads its first page; leaving
 // one drops the accumulated pages so memory resets between sections.
 useEffect(() => { if (['home', 'browse', 'search', 'tracker'].includes(route) && remoteTitles.length === 0) void fetchPage('').catch(() => { }) }, [route]);
 ...
 const submitSearch = async () => { const value = draftQuery.trim(); setSearching(true); try { if (value) { const page = await api.searchTitles(value); setRemoteTitles(page.items); setNextCursor(page.nextCursor); void api.ensureMetadata(page.items.map(item => item.id)).catch(() => { }); } setQuery(value); } catch (error) { setDetailMessage((error as Error).message); } finally { setSearching(false) } };
 const loadMore = () => { if (loadingMore.current || !nextCursor) return; loadingMore.current = true; api.titles({ search: query.trim().length >= 3 ? query.trim() : undefined, category, sort, pageSize: 12, cursor: nextCursor }).then(page => { setRemoteTitles(current => [...current, ...page.items]); setNextCursor(page.nextCursor); void api.ensureMetadata(page.items.map(item => item.id)); }).catch(() => { }).finally(() => { loadingMore.current = false; }); };
 ...
 useEffect(() => {
  const titleId = detail?.title.kind === 'series' ? detail.title.id : ''; if (!titleId) return; let stopped = false; let running = false; const refresh = async () => {
   if (stopped || running) return; running = true; try {
    const next = await api.title(titleId); if (!stopped && detailRef.current?.title.id === titleId) { // The 3-second poll re-renders the whole detail screen; skip the state update when nothing changed so focus survives idle cycles.
     if (JSON.stringify(next) === JSON.stringify(detailRef.current)) return;
     setDetail(next); setDetailMessage(needsEpisodeExpansion(next) ? 'Preparing the individual episode list…' : '')
    }
   } catch (error) { if (!stopped) setDetailMessage((error as Error).message) } finally { running = false }
  }; const timer = window.setInterval(() => void refresh(), 3000); return () => { stopped = true; window.clearInterval(timer) }
 }, [api, detail?.title.id]);
 ...
 const chooseRoute = (next: TVRoute) => { if (!['home', 'browse', 'search', 'tracker'].includes(next)) { setRemoteTitles([]); setNextCursor(null); } setRoute(next); setDetail(null); setMenuOpen(false); window.setTimeout(() => focusElement(document.querySelector<HTMLElement>('[data-focus-region="content"]')), 0); };
 ...
 // the card and the hint says so — the URL is the fallback.
 const openProjectLink = async (link: PortalState['links'][number]) => {
  const launched = await openExternalURL(link.url);
  if (!catalogMounted.current) return;
  if (launched) setProjectsMessage('');
  else setProjectsMessage('Open this address on another device: ' + link.url);
 };
 ...
 useTVNavigation({
  getInitialFocus: () => first.current || document.querySelector<HTMLElement>('[data-focus-region="content"]'), restoreKey: restoreFocus, onFocusKey: key => { onFocus(key); if (document.activeElement instanceof HTMLElement && document.activeElement.dataset.focusRegion === 'content') lastContent.current = key; }, onBack, onLongBack: exitApplication, onDirection: (direction, current) => {
   ...
   // Near the end of a title rail, fire the next page without consuming the
   // keypress: focus still moves while the append lands.
   if (direction === 'right' && (current.dataset.focusKey || '').startsWith('title-') && nearEnd(Number(current.dataset.focusCol || -1), visible.length)) loadMore();
   if (route === 'jobs' && direction === 'down' && (current.dataset.focusKey || '').startsWith('job-')) window.dispatchEvent(new CustomEvent('tv-jobs-near-end'));
 ...
 const heading = menuGroups.reduce<Array<{ id: TVRoute; label: string; icon: string }>>((items, group) => items.concat(group.items), []).find(item => item.id === route)?.label || (route === 'projects' ? 'Other projects' : 'Home');
 const hero = visible[0] || titles[0];
 const rows: Array<{ key: string; title: string; items: CatalogTitle[] }> = route === 'home' ? [
  { key: 'new', title: 'Recently added', items: remoteTitles },
 ] : route === 'tracker' ? [{ key: 'tracker-new', title: 'Recently added', items: visible }, { key: 'tracker-seeded', title: 'Strong swarms', items: [...visible].sort((a, b) => b.bestSeeders - a.bestSeeders) }] : ['continue', 'favorites', 'watched', 'library', 'downloads', 'library-categories', 'jobs', 'events', 'settings', 'categories'].includes(route) ? [] : [{ key: route, title: heading, items: visible }];
...
type PendingDownloadRemoval = { download: Download; scope: 'torrent' | 'file' };
function TVDownloads({ items, onPlay, onManage }: { items: Download[]; onPlay: (download: Download) => void; onManage: (download: Download, action: string) => Promise<void> }) {
 const [pending, setPending] = useState<PendingDownloadRemoval | null>(null);
 ...
 const closeConfirmation = () => { if (removing) return; const key = pending ? (pending.scope === 'file' ? `download-${pending.download.id}-file-${pending.download.fileIndex}-remove` : `download-${pending.download.id}-delete`) : ''; setPending(null); window.setTimeout(() => focusElement(document.querySelector<HTMLElement>(`[data-focus-key="${key}"]`) || document.querySelector<HTMLElement>('[data-focus-key="downloads-search"]')), 0) };
 useEffect(() => {
  if (!pending) return;
 ...
 const groups = useMemo(() => groupDownloadsByTorrent(visible), [visible]);
 const toggleFiles = (engineId: string) => setExpandedGroups(current => { const next = new Set(current); if (next.has(engineId)) next.delete(engineId); else next.add(engineId); return next });
 ...
 const confirmRemoval = async () => { if (!pending || removing) return; setRemoving(true); try { await onManage(pending.download, pending.scope === 'file' ? 'remove-file' : 'remove'); setPending(null); window.setTimeout(() => focusElement(document.querySelector<HTMLElement>('[data-focus-key="downloads-search"]')), 0) } finally { setRemoving(false) } };
 if (pending) return <section role="dialog" aria-modal="true" aria-labelledby="download-delete-heading" aria-describedby="download-delete-description" class="tv-settings tv-removal-confirm">{pending.scope === 'file' ? <><h2 id="download-delete-heading">Remove episode from downloads?</h2><strong>{pending.download.displayTitle || pending.download.filePath}</strong><p id="download-delete-description">The episode leaves the download list. Its file stays on disk until the whole torrent is deleted.</p></> : <><h2 id="download-delete-heading">Delete download?</h2><strong>{pending.download.releaseName || pending.download.filePath}</strong><p>Selected file: {pending.download.filePath}</p><p>{formatBytes(pending.download.sizeBytes)} · {pending.download.trackerName} release {pending.download.releaseId}</p><p id="download-delete-description">This removes the torrent from qBittorrent and permanently deletes its incomplete and downloaded files.</p></>}<button ref={cancelButton} disabled={removing} data-focus-region="download-dialog" data-focus-row="1" data-focus-col="0" data-focus-key="download-cancel" onClick={closeConfirmation}>Cancel</button><button ref={confirmButton} disabled={removing} class="danger-button" data-focus-region="download-dialog" data-focus-row="2" data-focus-col="0" data-focus-key="download-confirm" onClick={() => void confirmRemoval()}>{removing ? (pending.scope === 'file' ? 'Removing…' : 'Deleting…') : pending.scope === 'file' ? 'Remove episode' : 'Delete download'}</button></section>;
 ...
 return <section class="tv-list tv-download-list"><h2>Application downloads</h2><div class="tv-filters tv-download-tools"><input readOnly data-focus-region="content" data-focus-row="1" data-focus-col="0" data-focus-key="downloads-search" value={query} onInput={event => setQuery(event.currentTarget.value)} placeholder="Search downloads; press OK to type" /><button data-focus-region="content" data-focus-row="1" data-focus-col="1" data-focus-key="downloads-filter" onClick={() => cycle(filter, ['all', 'streaming', 'complete', 'paused', 'errors'], setFilter)}>Filter: {filterLabel}</button><button data-focus-region="content" data-focus-row="1" data-focus-col="2" data-focus-key="downloads-sort" onClick={cycleSort}>Sort: {sortLabel}</button></div><p class="tv-download-count" aria-live="polite">{groups.length} of {items.length} downloads shown</p>{groups.length === 0 ? <p>{items.length === 0 ? 'No managed downloads yet.' : 'No downloads match this search and filter.'}</p> : groups.map((group, groupIndex) => {
  const download = group.representative;
  const band = 2 + groupIndex * 20;
  const expanded = expandedGroups.has(download.engineId);
  const multi = group.rows.length > 1;
  return <article key={download.id} data-download-id={download.id} class="tv-download-row"><div><strong title={download.displayTitle || download.filePath}>{download.displayTitle || download.filePath}</strong><span class="tv-release-name" title={download.releaseName || download.filePath}>{download.releaseName || download.filePath}</span><span class={`tv-stream-mode ${download.playbackMode}`}>{download.playbackMode === 'progressive' ? 'Progressive stream' : 'Downloaded file'}</span><small>Tracker: {download.trackerName}</small><small>{[download.parsed?.resolution, download.parsed?.quality, download.parsed?.videoCodec, download.parsed?.audio, download.category].filter(Boolean).join(' · ') || 'Source details unavailable'}</small>{multi ? <small>{group.rows.length} files · {formatBytes(group.totalSizeBytes)}</small> : <small title={download.filePath}>Selected file: {download.filePath} · index {download.fileIndex} · {formatBytes(download.sizeBytes)}</small>}<small>Complete torrent: {download.trackerName} release {download.releaseId} · {download.trackerSeeders ?? '—'} tracker seeders{download.releaseSizeBytes ? ` · ${formatBytes(download.releaseSizeBytes)} total` : ''}</small><span class="tv-download-telemetry">{download.state} · {(download.progress * 100).toFixed(1)}% · {formatBytes(download.downloadedBytes)} / {formatBytes(download.sizeBytes)} selected</span><small class="tv-download-telemetry">{formatBytes(download.speedBytesPerSecond)}/s · {download.seeds} connected seeds · {download.peers} peers</small><small class={`tv-download-error ${download.error ? 'visible' : ''}`}>{download.error || 'No download error'}</small></div><div><button data-focus-region="content" data-focus-row={band} data-focus-col="0" data-focus-key={`download-${download.id}-play`} class="primary" onClick={() => onPlay(download)}>Play</button>{downloadTransferActions(download).map(item => <button key={item.action} disabled={busyAction.startsWith(download.id + ':')} data-focus-region="content" data-focus-row={band} data-focus-col="1" data-focus-key={`download-${download.id}-${item.action}`} onClick={() => void runAction(download, item.action)}>{busyAction === download.id + ':' + item.action ? item.pendingLabel : item.label}</button>)}<button class="danger-button" data-focus-region="content" data-focus-row={band} data-focus-col="2" data-focus-key={`download-${download.id}-delete`} onClick={() => setPending({ download, scope: 'torrent' })}>Delete download</button>{multi && <button data-focus-region="content" data-focus-row={band} data-focus-col="3" data-focus-key={`download-${download.id}-files`} aria-expanded={expanded} onClick={() => toggleFiles(download.engineId)}>{expanded ? 'Hide files' : 'Show files'}</button>}</div>{multi && expanded && <div class="tv-download-files">{group.rows.map((row, rowIndex) => <div key={row.id} class="tv-download-file"><strong title={row.displayTitle || row.filePath}>{row.displayTitle || row.filePath}</strong><small>{formatBytes(row.sizeBytes)}</small><span><button data-focus-region="content" data-focus-row={band + 1 + rowIndex} data-focus-col="0" data-focus-key={`download-${row.id}-file-${row.fileIndex}-play`} onClick={() => onPlay(row)}>Play</button><button class="danger-button" data-focus-region="content" data-focus-row={band + 1 + rowIndex} data-focus-col="1" data-focus-key={`download-${row.id}-file-${row.fileIndex}-remove`} onClick={() => setPending({ download: row, scope: 'file' })}>Remove</button></span></div>)}</div>}</article>;
 })}</section>
}
...
function TVJobs({ api, items: initial }: { api: API; items: Job[] }) {
 const [items, setItems] = useState(initial); const [query, setQuery] = useState(''); const [state, setState] = useState(''); const [kind, setKind] = useState(''); const [retryable, setRetryable] = useState(''); const [updatedHours, setUpdatedHours] = useState(''); const [next, setNext] = useState<string | null>(null); const [message, setMessage] = useState(''); const [detail, setDetail] = useState<{ job: Job; logs: JobLog[]; next: string | null } | null>(null);
 async function load(target = '', append = false) { try { const page = await api.jobs({ search: query, state, kind, retryable, updatedHours, pageSize: 12, cursor: target }); setItems(value => append ? [...value, ...page.items] : page.items); setNext(page.nextCursor) } catch (error) { setMessage((error as Error).message) } }
 ...
 useEffect(() => { const timer = window.setTimeout(() => { void load('') }, 400); return () => clearTimeout(timer) }, [query, state, kind, retryable, updatedHours]);
 useEffect(() => {
  // Catalog signals a jobs keypress near the list end; appends are guarded
  // so rapid keypresses cannot double-fetch the same page.
  let busy = false;
  const onNear = () => { if (busy || !next) return; busy = true; api.jobs({ search: query, state, kind, retryable, updatedHours, pageSize: 12, cursor: next }).then(page => { setItems(value => [...value, ...page.items]); setNext(page.nextCursor) }).catch(error => setMessage((error as Error).message)).finally(() => { busy = false }) };
  window.addEventListener('tv-jobs-near-end', onNear);
  return () => window.removeEventListener('tv-jobs-near-end', onNear);
 });
...
// creative's click URL to the platform browser (the server resolves
// tracking and redirects), and the address stays on the tile as the
// fallback for boxes without a browser. A hidden, failed, or empty
// delivery still leaves nothing in the DOM; delivery advances after each
// creative's screen time and stops on unmount or a hidden document.
function TVPromotions({ api }: { api: API }) {
 ...
 const url = api.promotionClickURL(promotion.provider, promotion.id);
 return <button class="tv-promo" data-focus-region="content" data-focus-row="95" data-focus-col="0" data-focus-key="promo" onClick={() => { void openExternalURL(url) }}><small>Advertisement</small>{/^https?:\/\//.test(promotion.image) && <img src={promotion.image} alt="" />}{promotion.title && <strong>{promotion.title}</strong>}{promotion.text && <p>{promotion.text}</p>}{promotion.link ? <code>{promotion.link}</code> : null}</button>;
}
...
function TVSeasonPackCard({ source, season, index, baseRow, open, onToggle, onAction, onDelete }: { source: CatalogSource; season: number; index: number; baseRow: number; open: boolean; onToggle: () => void; onAction: (source: CatalogSource, season: number, action: SeasonPackAction) => Promise<void | boolean>; onDelete: () => void }) {
 ...
 return <article class={`season-pack-card ${open ? 'expanded' : ''}`}>
  ...
  {open && <div id={`tv-pack-${source.release.id}`} class="season-pack-controls"><progress value={state?.progress || 0} max="1" aria-label="Season download progress" />{!managed && <button class="primary" disabled={Boolean(busy)} data-focus-region="content" data-focus-row={row + 1} data-focus-col="0" data-focus-key={`season-pack-${source.release.id}-download`} onClick={() => void run('download')}>{busy ? 'Starting…' : 'Download season'}</button>}{managed && !complete && !error && <button disabled={Boolean(busy)} data-focus-region="content" data-focus-row={row + 1} data-focus-col="1" data-focus-key={`season-pack-${source.release.id}-toggle`} onClick={() => void run(paused ? 'resume' : 'pause')}>{busy ? `${paused ? 'Resuming' : 'Pausing'}…` : paused ? 'Resume' : 'Pause'}</button>}{error && <button class="primary" disabled={Boolean(busy)} data-focus-region="content" data-focus-row={row + 1} data-focus-col="2" data-focus-key={`season-pack-${source.release.id}-retry`} onClick={() => void run('retry')}>{busy ? 'Retrying…' : 'Retry'}</button>}{managed && <button class="danger-button" disabled={Boolean(busy)} data-focus-region="content" data-focus-row={row + 1} data-focus-col="3" data-focus-key={`season-pack-${source.release.id}-delete`} onClick={onDelete}>Delete download</button>}</div>}
 </article>;
}
