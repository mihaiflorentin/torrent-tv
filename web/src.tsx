...
type NavItem = { id?: View; label: string; icon: string; href?: string };
...
export function App() {
  ...
  // Acquisition UX: a source click names its tracker, shows a preparing or
  // metadata state, and stays cancellable until the server returns a
  // Download. The UI never switches to playback early; cancellation is
  // distinct from failure, and 409/504 details render verbatim because the
  // server wording is already actionable.
  const [preparing, setPreparing] = useState<{ trackerId?: string; trackerName: string; label: string; canceled?: boolean; error?: string; settingsHint?: boolean } | null>(null);
  const [searchTrackers, setSearchTrackers] = useState<TrackerStatus[]>([]);
  const prepareAbort = useRef<AbortController | null>(null);
  useEffect(() => () => prepareAbort.current?.abort(), []);
  const beginPrepare = (trackerName: string, label: string, trackerId?: string) => { prepareAbort.current?.abort(); const controller = new AbortController(); prepareAbort.current = controller; setPreparing({ trackerId, trackerName, label }); return controller };
  const cancelPrepare = () => prepareAbort.current?.abort();
  const notePrepareFailure = (origin: { trackerName: string; label: string }, controller: AbortController) => (error: unknown) => {
    if (prepareAbort.current !== controller) return;
    if (controller.signal.aborted || (error as DOMException)?.name === 'AbortError') { setPreparing({ ...origin, canceled: true }); return }
  ...
  useEffect(() => { if (!loadMore.current) return; const observer = new IntersectionObserver(entries => { if (entries[0]?.isIntersecting && nextCursor && !loading) void loadTitles(true, nextCursor) }, { rootMargin: '500px' }); observer.observe(loadMore.current); return () => observer.disconnect() }, [view, nextCursor, loading, query, category, kind, resolution, sort]);
  ...
  async function playLegacy(item: HouseholdItem) { const origin = { trackerId: item.release.trackerId, trackerName: item.release.trackerName || 'Unknown tracker', label: item.release.name }; const controller = beginPrepare(origin.trackerName, origin.label, origin.trackerId); try { const d = await api.prepare(item.release.id, item.fileIndex, controller.signal); if (prepareAbort.current !== controller) return; const resumeMs = item.watched ? 0 : item.positionMs; setPreparing(null); setPlayer({ download: d, resumeMs }); pushRoute(watchRoute(d, resumeMs)) } catch (e) { notePrepareFailure(origin, controller)(e) } }
  ...
  async function downloadSeason(source: CatalogSource, season: number) { const origin = { trackerId: source.release.trackerId, trackerName: source.release.trackerName || 'Unknown tracker', label: source.release.name }; const controller = beginPrepare(origin.trackerName, origin.label, origin.trackerId); try { await api.prepareSeason(source.release.id, season, controller.signal); if (prepareAbort.current !== controller) return; setPreparing(null); await loadDownloads(); if (detailRef.current) { const next = await api.title(detailRef.current.title.id); setDetail(next); setDetailTarget({ season }) } } catch (e) { notePrepareFailure(origin, controller)(e) } }
  ...
  async function removeDownloadFile(d: Download) { try { await api.call(`/downloads/${encodeURIComponent(d.id)}/remove-file`, { method: 'POST' }); await loadDownloads() } catch (e) { setError((e as Error).message); throw e } }
  ...
  async function submitSearch(event?: Event, valueOverride?: string) { event?.preventDefault(); const value = (valueOverride ?? draftQuery).trim(); if (value === query) { void loadTitles(false, ''); return } setSearching(true); setError(''); try { if (value) { const page = await api.searchTitles(value); setTitles(page.items); setNextCursor(page.nextCursor); setHero(page.items[0] || null); setSearchTrackers(page.trackers || []); void api.ensureMetadata(page.items.slice(0, 12).map(item => item.id)).catch(() => { }); } setQuery(value); setUpdatesAvailable(false); if (view === 'search') { replaceRoute({ view: 'search', query: value }) } else { setView('search'); pushRoute({ view: 'search', query: value }) } } catch (e) { setError((e as Error).message) } finally { setSearching(false) } }
  ...
  return <div class="app-shell"><Sidebar view={view} onView={guardedNavigate} dock={<PortalSidebarDock snapshot={portal} client={api} identity={identity} onOpenAccount={() => setAccountOpen(true)} openExternal={openExternal} />} /><main class="content" id="content"><header class="topbar"><div><h1>{pageTitle}</h1><p>{view === 'home' ? 'Your private screening archive' : view === 'browse' ? 'Every title, grouped and ready to compare' : ''}</p></div><button class="avatar" aria-label="Household profile">H</button></header>{error && <div class="error" role="alert"><strong>Something needs attention</strong><span>{error}</span><button onClick={() => setError('')}>Dismiss</button></div>}{updateStatus?.available && <UpdateNotice status={updateStatus} controller={updates} openExternal={openExternal} />}
    ...
    {showCatalog && <><CatalogTools draftQuery={draftQuery} setDraftQuery={setDraftQuery} query={query} searching={searching} onSubmit={submitSearch} category={category} setCategory={setCategory} kind={kind} setKind={setKind} resolution={resolution} setResolution={setResolution} sort={sort} setSort={setSort} facets={facets} trackers={searchTrackers} />{updatesAvailable && <button class="catalog-update" onClick={() => void applyUpdates()}>Catalog updates available · Refresh</button>}{view === 'categories' ? <CategoryGrid categories={facets.categories} onSelect={c => { setCategory(c); navigate('browse') }} /> : view === 'tracker' ? <><Rail title="Recently added" onNearEnd={loadMoreTitles}>{titles.map(t => <MediaCard title={t} onOpen={openTitle} />)}</Rail><Rail title="Strong swarms" onNearEnd={loadMoreTitles}>{[...titles].sort((a, b) => b.bestSeeders - a.bestSeeders).map(t => <MediaCard title={t} onOpen={openTitle} />)}</Rail></> : <><section class="poster-grid" aria-busy={loading}>{titles.map(t => <MediaCard title={t} onOpen={openTitle} />)}</section>{loading && <section class="poster-grid">{Array.from({ length: 12 }, (_, i) => <div class="skeleton" key={i} />)}</section>}<div ref={loadMore} class="load-more" aria-hidden="true" /></>}</>}
    {view === 'downloads' &&
...
function Detail({ detail, target, resume, favorite, onClose, onPlay, onResume, onSource, onPackAction, onFavorite, onError }: { detail: CatalogDetail; target: DetailTarget; resume?: HouseholdItem; favorite: boolean; onClose: () => void; onPlay: () => void; onResume: (item: HouseholdItem) => void; onSource: (s: CatalogSource) => void; onPackAction: (s: CatalogSource, season: number, action: SeasonPackAction) => Promise<void>; onFavorite: (v: boolean) => void; onError: (message: string) => void }) {
  ...
  const season = detail.seasons[t];
  const firstSource = detail.title.kind === 'movie' ? detail.sources[0] : season?.episodes[0]?.sources[0];
  const canPlay = Boolean(firstSource);
  const unknownKey = season ? `unknown:${season.number}` : '';
  const unknownOpen = expanded === unknownKey;
  useOverlayFocus(Boolean(pendingPack), () => { if (!deleting) setPendingPack(null) });
  const confirmDelete = async () => { if (!pendingPack || !season || deleting) return; setDeleting(true); try { await onPackAction(pendingPack, season.number, 'delete'); setPendingPack(null) } finally { setDeleting(false) } };
  // Refresh re-runs the whole pipeline with force: provider data and tracker
  // search (new sources, fresh seeds/peers), then the view rebuilds from the
  // existing catalog.updated SSE listener and 3s series polling.
  const refresh = async () => {
    if (refreshing) return;
    setRefreshing(true);
    try { await api.refreshTitle(detail.title.id, detail.title.title, true) } catch (e) { onError((e as Error).message) } finally { setRefreshing(false) }
  };
  const episodeRow = (e: CatalogEpisode) => {
    const key = `${e.season}:${e.number}`;
    const open = expanded === key;
    return <article class={open ? 'expanded' : ''}><button class="episode-select" aria-expanded={open} onClick={() => setExpanded(current => current === key ? '' : key)}><div class="episode-art"><Artwork title={detail.title} kind="backdrop" /></div><span class="episode-copy"><strong>{e.number ? `${e.number}. ` : ''}{e.title}</strong><small>{e.sourceCount} version{e.sourceCount === 1 ? '' : 's'}</small><MediaBadges state={e.libraryState} /><b>{open ? 'Hide versions' : 'Show versions'}</b></span></button>{open && <SourceRows sources={e.sources} onSource={onSource} compact />}</article>;
  };
  return <div class="overlay" role="dialog" aria-modal="true" aria-label={`${detail.title.title} details`}><article class="detail"><button class="close" onClick={onClose}>Close</button><div class="detail-hero"><Artwork title={detail.title} kind="backdrop" /><div /><section><h2>{detail.title.title}</h2><p>{[detail.title.year, detail.title.kind === 'series' ? `${detail.title.seasonCount} seasons` : 'Movie', `${detail.title.bestSeeders} seeds`].filter(Boolean).join(' · ')}</p>{detail.title.genres && detail.title.genres.length > 0 && <p class="title-genres">{detail.title.genres.join(' · ')}</p>}<p class="tracker-names">{detail.title.trackers.map(tracker => <TrackerBadge key={tracker.id} id={tracker.id} name={tracker.name} />)}</p><MediaBadges state={detail.title.libraryState} /><p>{detail.title.overview || 'Metadata is still being prepared.'}</p><div class="actions">{resume ? <button class="primary" onClick={() => onResume(resume)} aria-label={`${resumeActionLabel(resume, detail.title.kind)} at saved position`}><Icon name="play" />{resumeActionLabel(resume, detail.title.kind)}</button> : canPlay && <button class="primary" onClick={onPlay}><Icon name="play" />{firstSource ? sourceActionLabel(firstSource) : 'Play'}</button>}<button onClick={() => void onFavorite(!favorite)}><Icon name="heart" />{favorite ? 'Remove favorite' : 'Favorite'}</button><button disabled={refreshing} title="Re-fetches series data from the metadata providers and searches all trackers again, then rebuilds this episode list." onClick={() => void refresh()}><Icon name="refresh" />{refreshing ? 'Refreshing…' : 'Refresh data'}</button></div>{resume && <small class="resume-file">{resumeSummary(resume, detail.title.kind)}</small>}</section></div>{detail.title.kind === 'series' ? <><div class="season-tabs">{detail.seasons.map((s, i) => <button class={i === t ? 'selected' : ''} onClick={() => { setT(i); setExpanded(''); setExpandedPack('') }}><span>{s.title}</span><MediaBadges state={s.libraryState} /></button>)}</div>{season?.packSources && season.packSources.length > 0 && <section class="season-pack-actions"><button class="packs-toggle" aria-expanded={packsOpen} onClick={() => setPacksOpen(current => !current)}><span class="packs-toggle-copy"><strong>Complete season versions ({season.packSources.length})</strong><small>{packsOpen ? 'Select a version to review it. Downloads start only from the button inside the expanded version.' : 'Show the complete season downloads.'}</small></span><b>{packsOpen ? 'Hide' : 'Show'}</b></button>{packsOpen && <div>{season.packSources.map(source => <SeasonPackCard key={source.release.id} source={source} season={season.number} open={expandedPack === source.release.id} onToggle={() => setExpandedPack(current => current === source.release.id ? '' : source.release.id)} onAction={onPackAction} onDelete={() => setPendingPack(source)} />)}</div>}</section>}{season && season.episodes.length > 0 ? <div class="episode-list">{season.episodes.map(e => episodeRow(e))}</div> : <p class="episode-loading" role="status">Preparing the individual episode list. This page updates automatically when it is ready.</p>}{season?.unknown && season.unknown.length > 0 && <section class="unknown-group"><button class="unknown-header" aria-expanded={unknownOpen} onClick={() => setExpanded(current => current === unknownKey ? '' : unknownKey)}><strong>Unknown episodes ({season.unknown.length})</strong><span>Files that did not match the metadata provider</span><b>{unknownOpen ? 'Hide' : 'Show'}</b></button>{unknownOpen && <div class="episode-list">{season.unknown.map(e => episodeRow(e))}</div>}</section>}</> : <SourceRows sources={detail.sources} onSource={onSource} />}</article>{pendingPack && <div class="overlay" role="dialog" aria-modal="true" aria-labelledby="season-pack-delete-heading"><section class="removal-confirm"><h2 id="season-pack-delete-heading">Delete season download?</h2><p class="release-name">{pendingPack.release.name}</p><p class="tracker-name"><TrackerBadge id={pendingPack.release.trackerId} name={pendingPack.release.trackerName} /></p><p>This removes the shared season torrent from qBittorrent and permanently deletes every episode file in it.</p><div class="confirm-actions"><button disabled={deleting} onClick={() => setPendingPack(null)}>Cancel</button><button class="danger-button" disabled={deleting} onClick={() => void confirmDelete()}>{deleting ? 'Deleting…' : 'Delete download'}</button></div></section></div>}</div>;
}
function SourceRows({ sources, onSource, compact = false }: { sources: CatalogSource[]; onSource: (s: CatalogSource) => void; compact?: boolean }) { const ordered = [...sources].sort((a, b) => { const rank = (source: CatalogSource) => source.libraryState?.downloadState === 'downloaded' ? 0 : source.libraryState?.downloadState && source.libraryState.downloadState !== 'none' ? 1 : 2; return rank(a) - rank(b) || b.release.seeders - a.release.seeders }); return <div class={`source-rows ${compact ? 'compact' : ''}`}>{ordered.map(s => <button onClick={() => onSource(s)}><span class="source-copy"><strong>{s.parsed.resolution || 'Source'}</strong><span class="source-filename">{s.filePath || s.release.name}</span><small>{[s.parsed.hdr, s.parsed.quality, s.parsed.videoCodec].filter(Boolean).join(' · ')}</small><TrackerBadge id={s.release.trackerId} name={s.release.trackerName} /></span><span class="source-action"><MediaBadges state={s.libraryState} /><b>{sourceActionLabel(s)}</b><small>{formatBytes(s.fileSizeBytes || s.release.sizeBytes)} · {s.release.seeders} seeds</small></span></button>)}</div> }
