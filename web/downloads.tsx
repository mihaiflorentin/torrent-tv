// The Downloads view: search/filter/sort toolbar, one card per torrent
// (episode files of the same engine route grouped), and the removal
// confirms. Extracted verbatim from src.tsx so the desktop GUI can mount
// it; data flows in through props and action callbacks, so the component
// needs no API client of its own. The modal focus machinery below is
// duplicated from src.tsx (pending a shared module) because the removal
// confirm owns an Escape-closable overlay.
import { useEffect, useMemo, useRef, useState } from 'preact/hooks';
import { Download, DownloadSort, downloadTransferActions, DownloadTransferAction, formatBytes, groupDownloadsByTorrent, orderDownloadIDs } from '@torrent-tv/shared';
...
type PendingRemoval = { download: Download; scope: 'torrent' | 'file' };
export function Downloads({ items, onRefresh, onPlay, onRemove, onRemoveFile, onAction }: { items: Download[]; onRefresh: () => void; onPlay: (d: Download) => void; onRemove: (d: Download) => Promise<void>; onRemoveFile: (d: Download) => Promise<void>; onAction: (d: Download, action: DownloadTransferAction) => Promise<void> }) {
  const [pending, setPending] = useState<PendingRemoval | null>(null); const [removing, setRemoving] = useState(false); const [query, setQuery] = useState(''); const [filter, setFilter] = useState('all'); const [sort, setSort] = useState<DownloadSort>('recent'); const [order, setOrder] = useState<string[]>(() => orderDownloadIDs(items, 'recent')); useOverlayFocus(Boolean(pending), () => { if (!removing) setPending(null) });
  ...
  const visible = useMemo(() => { const term = query.trim().toLocaleLowerCase(); const byID = new Map(items.map(item => [item.id, item])); const ordered = (order.length ? order : items.map(item => item.id)).map(id => byID.get(id)).filter((item): item is Download => Boolean(item)); return ordered.filter(download => { const text = [download.displayTitle, download.releaseName, download.filePath, download.category, download.state, download.trackerName].filter(Boolean).join(' ').toLocaleLowerCase(); const matchesFilter = filter === 'all' || filter === 'streaming' && download.playbackMode === 'progressive' || filter === 'complete' && download.playbackMode === 'local' || filter === 'paused' && /^(paused|stopped)/.test(download.state.toLocaleLowerCase()) || filter === 'errors' && Boolean(download.error); return (!term || text.includes(term)) && matchesFilter }) }, [items, order, query, filter]);
  // One card per physical torrent: predicates stay per-row, a group shows
  // when any of its rows matches, and the expanded file list carries the
  ...
  const confirmRemoval = async () => { if (!pending || removing) return; setRemoving(true); try { await (pending.scope === 'file' ? onRemoveFile : onRemove)(pending.download); setPending(null) } finally { setRemoving(false) } };
  return <section><div class="catalog-tools download-tools"><label class="search"><Icon name="search" /><input value={query} onInput={event => setQuery(event.currentTarget.value)} placeholder="Search downloaded titles, releases, or files" aria-label="Search downloads" /></label><select value={filter} onChange={event => setFilter(event.currentTarget.value)} aria-label="Filter downloads"><option value="all">All downloads</option><option value="streaming">Still downloading</option><option value="complete">Downloaded</option><option value="paused">Paused</option><option value="errors">Needs attention</option></select><select value={sort} onChange={event => changeSort(event.currentTarget.value as DownloadSort)} aria-label="Sort downloads"><option value="recent">Recently added</option><option value="title">Title A–Z</option><option value="progress">Most progress</option><option value="size">Largest file</option><option value="speed">Fastest download</option></select><button onClick={onRefresh}>Refresh</button><span class="search-scope" aria-live="polite">{groups.length} of {items.length} downloads shown</span></div><div class="download-list">{groups.length === 0 ? <p class="empty">{items.length === 0 ? 'Downloads you start will appear here.' : 'No downloads match this search and filter.'}</p> : groups.map(group => { const download = group.representative; const expanded = expandedGroups.has(download.engineId); const multi = group.rows.length > 1; return <article key={download.id} data-download-id={download.id}><div class="download-identity"><h2 title={download.displayTitle || download.filePath}>{download.displayTitle || download.filePath}</h2><p class="release-name" title={download.releaseName || download.filePath}>{download.releaseName || download.filePath}</p><span class={`stream-mode ${download.playbackMode}`}>{download.playbackMode === 'progressive' ? 'Progressive stream' : 'Downloaded file'}</span><dl>{multi ? <div><dt>Files</dt><dd>{group.rows.length} files · {formatBytes(group.totalSizeBytes)} <button class="files-toggle" onClick={() => toggleFiles(download.engineId)}>{expanded ? 'Hide files' : 'Show files'}</button></dd></div> : <div><dt>Selected file</dt><dd title={download.filePath}>{download.filePath}</dd></div>}<div><dt>Source</dt><dd>{facts(download) || 'Source details unavailable'}</dd></div><div><dt>Selected file size</dt><dd>{formatBytes(download.sizeBytes)} · index {download.fileIndex}</dd></div><div><dt>Complete torrent</dt><dd>{download.releaseId} · {download.trackerSeeders ?? '—'} tracker seeders{download.releaseSizeBytes ? ` · ${formatBytes(download.releaseSizeBytes)} total` : ''}</dd></div><div><dt>Tracker</dt><dd><TrackerBadge id={download.trackerId} name={download.trackerName} /></dd></div></dl>{multi && expanded && <ul class="download-files">{group.rows.map(row => <li key={row.id}><span class="download-file-title" title={row.displayTitle || row.filePath}>{row.displayTitle || row.filePath}</span><span class="download-file-size">{formatBytes(row.sizeBytes)}</span><span class="download-file-actions"><button onClick={() => onPlay(row)}>Play</button><button class="danger-button" onClick={() => setPending({ download: row, scope: 'file' })}>Remove</button></span></li>)}</ul>}<p class="download-telemetry">{download.state} · {(download.progress * 100).toFixed(1)}% · {formatBytes(download.downloadedBytes)} / {formatBytes(download.sizeBytes)} selected</p><progress value={download.progress} max="1" /><p class="download-telemetry"><span class={`engine-tag ${download.engineId.startsWith('native:') ? 'native' : 'qbittorrent'}`}>{download.engineId.startsWith('native:') ? 'native engine' : 'qBittorrent'}</span> · {formatBytes(download.speedBytesPerSecond)}/s · {download.seeds} connected seeds · {download.peers} peers · {download.seeds + download.peers} known in swarm · {download.uploadSpeedBytesPerSecond ? `↑ ${formatBytes(download.uploadSpeedBytesPerSecond)}/s` : '↑ idle'} · {download.etaSeconds > 0 ? `ETA ${Math.max(1, Math.round(download.etaSeconds / 60))} min` : 'ETA —'}</p><p class={`download-error ${download.error ? 'visible' : ''}`} aria-live="polite">{download.error}</p></div><div class="download-actions"><button class="primary" onClick={() => onPlay(download)}>Play</button>{downloadTransferActions(download).map(item => <button key={item.action} disabled={busy.startsWith(download.id + ':')} onClick={() => void runAction(download, item.action)}>{busy === download.id + ':' + item.action ? item.pendingLabel : item.label}</button>)}<button class="danger-button" onClick={() => setPending({ download, scope: 'torrent' })}>Delete download</button></div></article> })}</div>{pending && <div class="overlay" role="dialog" aria-modal="true" aria-labelledby="web-download-delete-heading"><section class="removal-confirm">{pending.scope === 'file' ? <><h2 id="web-download-delete-heading">Remove episode from downloads?</h2><p class="release-name">{pending.download.displayTitle || pending.download.filePath}</p><p>The episode leaves the download list. Its file stays on disk until the whole torrent is deleted.</p></> : <><h2 id="web-download-delete-heading">Delete download?</h2><p class="release-name">{pending.download.releaseName || pending.download.filePath}</p><dl><div><dt>Selected file</dt><dd>{pending.download.filePath}</dd></div><div><dt>Tracker</dt><dd><TrackerBadge id={pending.download.trackerId} name={pending.download.trackerName} /></dd></div><div><dt>Tracker release ID</dt><dd>{pending.download.releaseId}</dd></div><div><dt>Selected file size</dt><dd>{formatBytes(pending.download.sizeBytes)}</dd></div></dl><p>This removes the torrent from qBittorrent and permanently deletes its incomplete and downloaded files.</p></>}<div class="confirm-actions"><button disabled={removing} onClick={() => setPending(null)}>Cancel</button><button class="danger-button" disabled={removing} onClick={() => void confirmRemoval()}>{removing ? (pending.scope === 'file' ? 'Removing…' : 'Deleting…') : pending.scope === 'file' ? 'Remove episode' : 'Delete download'}</button></div></section></div>}</section>
}
